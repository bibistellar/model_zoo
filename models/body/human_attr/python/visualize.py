...
def visualize_attr(im, results, boxes=None, is_mtmct=False, font_size_ratio=60):
    """
    在图像上绘制属性文本（中文），默认使用系统宋体（SimSun）。
    
    Args:
        im (str or np.ndarray): 输入图像路径或者 BGR 格式的 NumPy 数组。
        results (List[List[str]]): 每个目标对应的一组文本（中文字符串）列表。
        boxes (List[tuple], optional): 每个目标的边界框。多摄像头时格式 (x, y, w, h)，
                                       单摄像头时格式 (0,0,x,y,w,h)。默认为 None。
        is_mtmct (bool): boxes 是否为多摄像头格式。默认为 False。
        font_size_ratio (int): 字体大小与图像高度的比例分母，用于动态调整字体。
    
    Returns:
        np.ndarray: 绘制完毕的 BGR 图像数组。
    """
    # 1. 加载图像
    if isinstance(im, str):
        im = Image.open(im)
    ...
    # 3. 将 OpenCV 图像转换为 PIL 对象
    img_pil = Image.fromarray(cv2.cvtColor(im, cv2.COLOR_BGR2RGB))
    draw = ImageDraw.Draw(img_pil)

    # 4. 动态计算字体大小
    font_size = max(12, int(im.shape[0] / font_size_ratio))
    font = ImageFont.truetype(font_path, font_size)

    # 5. 逐目标绘制文本
    line_spacing = im.shape[0] / 40.  # 行间距
    for i, res in enumerate(results):
        # 5.1 计算每组文本的起始坐标
        if boxes is None:
            x, y = 3, 1
        elif is_mtmct:
            x, y = int(boxes[i][0]) + 3, int(boxes[i][1])
        else:
            x, y = int(boxes[i][2]) + 3, int(boxes[i][3])
        y_off = y

        # 5.2 绘制该目标的多行文本
        for text in res:
            y_off += int(line_spacing)
            draw.text((x, y_off), text, font=font, fill=(255, 255, 15))

    # 6. 转回 OpenCV BGR 并返回
    return cv2.cvtColor(np.asarray(img_pil), cv2.COLOR_RGB2BGR)

def visualize_attr_en(im, results, boxes=None, is_mtmct=False):
    if isinstance(im, str):
        im = Image.open(im)
        im = np.ascontiguousarray(np.copy(im))
        im = cv2.cvtColor(im, cv2.COLOR_RGB2BGR)
