...
from cfg_utils import argsparser, print_arguments, merge_cfg, load_cfg
...
class PipePredictor(object):
    ...
    def run(self, input, thread_idx=0):
        ...
        input 可以是：
          - 单张图的 np.ndarray
          - 多张图组成的 list[np.ndarray]
          - 文件路径 list[str]
        """
        if isinstance(input, np.ndarray):
            imgs_list = [input]
        elif (isinstance(input, list) and len(input) > 0 and 
              isinstance(input[0], np.ndarray)):
            imgs_list = input
        else:
            imgs_list = [cv2.imread(f) for f in input]

        if self.is_video:
            self.predict_video(imgs_list, thread_idx=thread_idx)
        else:
            self.predict_image(imgs_list)
        ...
        results = {k: v for k, v in members if not k.startswith('_')}
        return results

    def predict_image(self, imgs_list):
        """
        imgs_list: list of np.ndarray, 每个 shape = (H, W, 3), BGR uint8
        """
        batch_size = self.det_predictor.batch_size
        total_imgs = len(imgs_list)
        batch_cnt = math.ceil(total_imgs / float(batch_size))
        self.warmup_frame = min(10, total_imgs // 2) - 1

        for i in range(batch_cnt):
            start = i * batch_size
            end = min(start + batch_size, total_imgs)
            batch_imgs = imgs_list[start:end]

            if i > self.warmup_frame:
                self.pipe_timer.total_time.start()
                self.pipe_timer.module_time['det'].start()
            # det output format: class, score, xmin, ymin, xmax, ymax
            det_res = self.det_predictor.predict_image(batch_imgs, visual=False)
            ...
            # ---------- 人体属性 ----------
            if self.with_human_attr:
                crop_inputs = crop_image_with_det(batch_imgs, det_res)
                attr_list = []
                if i > self.warmup_frame:
                    self.pipe_timer.module_time['attr'].start()
                for crop in crop_inputs:
                    ar = self.attr_predictor.predict_image(crop, visual=False)
                    attr_list.extend(ar['output'])
                if i > self.warmup_frame:
                    self.pipe_timer.module_time['attr'].end()
                self.pipeline_res.update({'output': attr_list}, 'attr')
            ...
            if self.with_vehicle_attr:
                ...
                for crop in crop_inputs:
                    vr = self.vehicle_attr_predictor.predict_image(crop, visual=False)
                    vattr_list.extend(vr['output'])
                if i > self.warmup_frame:
                    self.pipe_timer.module_time['vehicle_attr'].end()
                self.pipeline_res.update({'output': vattr_list}, 'vehicle_attr')
            ...
            if self.with_vehicleplate:
                ...
                plates = []
                for crop in crop_inputs:
                    pr = self.vehicleplate_detector.get_platelicense(crop)
                    plates.extend(pr['plate'])
                if i > self.warmup_frame:
                    self.pipe_timer.module_time['vehicleplate'].end()
                self.pipeline_res.update({'vehicleplate': plates}, 'vehicleplate')
            ...
            if self.with_vehicle_press:
                ...
                lanes, direction = self.laneseg_predictor.run(batch_imgs)
                if lanes:
                    self.pipeline_res.update({'output': lanes, 'direction': direction}, 'lanes')
                    press = self.vehicle_press_predictor.run(lanes, det_res)
                    self.pipeline_res.update({'output': press}, 'vehicle_press')
                else:
                    print("no lanes!")

                if i > self.warmup_frame:
                    self.pipe_timer.module_time['vehicle_press'].end()
            self.pipe_timer.img_num += len(batch_imgs)
            if i > self.warmup_frame:
            ...
            #     lambda a: not inspect.isroutine(a)
            # )
            # data = {k: v for k, v in members if not k.startswith('_')}
            # print(data)
...
def main():
    ...
    # pipeline.run()
    img = cv2.imread("/data/workspace/aiplat/saiserver-ftb/detect/pipeline/c.png", cv2.IMREAD_COLOR)
...
if __name__ == '__main__':
    ...
    FLAGS.config = "/data/workspace/aiplat/saiserver-ftb/detect/pipeline/config/examples/infer_cfg_human_attr.yml"
