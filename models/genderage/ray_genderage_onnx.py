...
class GenderAgeDetector:
    ...
    def __init__(self, model_file=None, tpu_id=-1):
        ...
        if model_file is None:
            current_dir = os.path.dirname(os.path.abspath(__file__))
            model_file = os.path.join(current_dir, "models/genderage/genderage.onnx")
    ...
    def forward(self, batch_data):
        ...
        Args:
            batch_data: 预处理好的批量图像数据 (np.ndarray)
            
        Returns:
            模型的原始输出
        """
        raw_out = self.session.run([self.output_name], {self.input_name: batch_data})[0]
        return raw_out
    ...
    def detect_genderage(self, image, face_bbox):
        """
        检测图像中指定人脸边界框内的性别和年龄
        ...
        try:
            # 检测性别和年龄
            gender, age = self.get(image, face_bbox)
            ...
            label = f"{gender_str}, {age}岁"
            cv2.putText(result_img, label, (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            
            return gender_str, age, result_img
        except Exception as e:
            print(f"性别年龄检测时发生错误: {str(e)}")
            import traceback
            traceback.print_exc()
...
try:
    actor = ray.get_actor("GenderAge", namespace="face_analysis")
    ray.kill(actor)
    print(f"已终止Actor: GenderAge")
except:
    print(f"Actor GenderAge 不存在或已终止")

# 部署新的Actor
deploy = GenderAgeDetector.options(
    name="GenderAge",
